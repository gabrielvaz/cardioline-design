...
import { FileText, ListFilter, Search, SlidersHorizontal } from 'lucide-react';
...
import { TableSettingsMenu, type TableDensity } from '@/components/ui/table-settings-menu';
...
const tableColumns = [{ id: 'id', label: 'Exam ID' }, { id: 'name', label: 'Patient name', locked: true }, { id: 'patientId', label: 'Patient ID' }, { id: 'date', label: 'Reception' }, { id: 'unit', label: 'Unit' }, { id: 'modifiedBy', label: 'Modified by' }, { id: 'type', label: 'Exam type' }, { id: 'result', label: 'Summary' }, { id: 'actions', label: 'Actions', locked: true }];

export default function ExamsPage() {
  const [query, setQuery] = React.useState('');
  const [advanced, setAdvanced] = React.useState(false);
  const [visibleColumns, setVisibleColumns] = React.useState(tableColumns.map((column) => column.id));
  ...
  return (
    <div className="space-y-6">
      <section className="space-y-5">
        ...
        <div className="overflow-x-auto pb-1">
          <div className="flex min-w-full w-max items-center gap-2 pr-1">
            ...
            <Button onClick={() => setToast('Report area opened.')} className="ml-auto h-11 shrink-0 bg-primary px-5 text-white">Report area</Button>
          </div>
        </div>

        <div className="flex flex-col justify-between gap-3 lg:flex-row lg:items-center">
          <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row">
            <div className="relative w-full sm:w-[310px]"><Search className="absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-gray-400" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Type to search..." className="h-11 border-gray-200 bg-white pl-10" /></div>
          ...
          <div className="flex items-center gap-3 self-end lg:self-auto">
            <TableSettingsMenu columns={tableColumns} visibleColumns={visibleColumns} onVisibleColumnsChange={setVisibleColumns} density={density} onDensityChange={setDensity} />
      ...
      <TanstackExamTable query={query} visibleColumns={visibleColumns} density={density} />
