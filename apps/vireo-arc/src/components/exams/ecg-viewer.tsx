...
export function EcgViewer({ exam }: { exam: Exam }) {
  ...
  const [gain, setGain] = React.useState(10); // mm/mV
  const [speed, setSpeed] = React.useState(25); // mm/s
  ...
  return <div className="-m-6 min-h-[calc(100vh-4rem)] bg-white text-slate-700">
    ...
    >
      <main className="flex min-h-[calc(100dvh-8.75rem)] min-w-0 flex-col xl:min-h-0"><div className="flex flex-wrap items-center gap-2 border-b border-slate-100 bg-white px-3 py-2"><Control label="Compare" /><Control label="Format: 12×1" /><div className="min-w-[104px]"><Label htmlFor="ecg-gain" className="sr-only">Gain (mm/mV)</Label><Select value={String(gain)} onValueChange={(value) => setGain(Number(value))}><SelectTrigger id="ecg-gain" aria-label="Gain (mm/mV)"><SelectValue>{`mm/mV: ${gain}`}</SelectValue></SelectTrigger><SelectContent>{[5, 10, 20].map((option) => <SelectItem key={option} value={String(option)}>{`mm/mV: ${option}`}</SelectItem>)}</SelectContent></Select></div><div className="min-w-[104px]"><Label htmlFor="ecg-speed" className="sr-only">Paper speed (mm/s)</Label><Select value={String(speed)} onValueChange={(value) => setSpeed(Number(value))}><SelectTrigger id="ecg-speed" aria-label="Paper speed (mm/s)"><SelectValue>{`mm/s: ${speed}`}</SelectValue></SelectTrigger><SelectContent>{[25, 50].map((option) => <SelectItem key={option} value={String(option)}>{`mm/s: ${option}`}</SelectItem>)}</SelectContent></Select></div><Control label="Muscular filter: on" /><button type="button" onClick={() => setShowMeasurements((visible) => !visible)} aria-expanded={showMeasurements} aria-controls="ecg-measurements" className="ml-2 inline-flex h-9 items-center gap-2 rounded-md px-2 text-sm font-medium text-slate-600 transition-colors hover:bg-orange-50 hover:text-[#ee5b00] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#ee5b00]/30"><ChevronUp className={cn('h-4 w-4 shrink-0 transition-transform duration-300 ease-out', !showMeasurements && 'rotate-180')} /> <span>{showMeasurements ? 'Hide measurements' : 'Show measurements'}</span></button></div>
        <div
          id="ecg-measurements"
        ...
        <Waveform fillAvailableHeight scaleX={speed / 25} scaleY={gain / 10} />
      </main>
...
function Waveform({ fillAvailableHeight = false, scaleX = 1, scaleY = 1 }: { fillAvailableHeight?: boolean; scaleX?: number; scaleY?: number }) { const pattern = '0,56 30,56 38,51 45,62 52,56 58,17 64,92 70,56 100,56 110,48 120,56 140,56'; const width = Math.round(1260 * scaleX); return <div className={cn('ecg-waveform ecg-waveform-surface relative min-h-[520px] overflow-auto p-5', fillAvailableHeight && 'ecg-waveform-expanded flex flex-1 flex-col')}>{['aVF','V1','V2','V3','V4','V5'].map((lead,index) => <div key={lead} className={cn('ecg-lead-row relative min-w-[1160px]', fillAvailableHeight ? 'flex-1' : 'h-20')}><span className="absolute left-0 top-1 text-xs font-bold text-slate-700">{lead}</span><svg className="h-full w-full text-slate-600 dark:text-slate-200" viewBox={`0 0 ${width} 112`} preserveAspectRatio="none"><polyline points={Array.from({ length: 9 }, (_, i) => pattern.split(' ').map((point) => { const [x,y] = point.split(',').map(Number); return `${Math.round((x + i * 140) * scaleX)},${Math.round(56 + (y - 56) * scaleY + index * (index % 2 ? 0 : 1))}`; }).join(' ')).join(' ')} fill="none" stroke="currentColor" strokeWidth="1.4" /></svg></div>)}</div>; }
